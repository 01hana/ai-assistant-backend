...
Likely modified files are the existing PageContext DTO/mapper, Assistant controller/message/runtime types and service, connector interface and mock module/adapter, ToolRegistry validation/policy interpretation, ToolCall summary handling, EvidenceRef input, and Assistant module imports/providers. Phase 1 propagation additionally narrows the existing Assistant session creation types/service, message repository, context-state input types, planning types/service, query-understanding types/service, entity extractor, deixis resolver, and PageContext-bearing approval/action/escalation input types so only `HostIntegrationContext` and `NormalizedPageContext` cross those internal boundaries.

Planning files have split ownership: Phase 1 changes only their trusted host and normalized PageContext inputs; Phase 3 retains candidate-operation restructuring, legacy-operation handling, and canonical ToolDefinition operation authority. QueryTaskDecomposer candidate generation also remains Phase 3 work.

Identity verification modules, Gateway, public controller routes, SDK, Prisma schema/migrations, retrieval, feedback, approval/action/escalation service behavior, and Customer-specific transport modules are explicitly not changed by this feature. Approval public behavior and workflow contracts remain unchanged; only their PageContext-bearing internal input contracts are narrowed to `NormalizedPageContext` so raw wire context cannot enter persistence-producing approval flows. Dependency direction remains acyclic: host integration depends on identity types; connectors depend on host-integration types and tools types; Assistant orchestrates all three; neither connectors nor host integration depends on Assistant services.
