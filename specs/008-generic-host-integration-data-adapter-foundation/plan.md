...
| **MODIFIED** `src/assistant/planning/assistant-planning.types.ts` | Defines persisted execution-plan candidate structures | Constrain candidates to safe `{ key, arguments, reason }` and exclude independent operation authority | 3 | High: planner operation injection | Candidate serialization and legacy-field tests |
| **MODIFIED** `src/assistant/planning/assistant-planning.service.ts` | Builds and persists ExecutionPlan from query understanding and page/context state | Persist generic validated candidate shape without transient state or native credentials | 3 | High: unsafe arguments/reference persistence | Planning persistence and secret-redaction integration tests |
